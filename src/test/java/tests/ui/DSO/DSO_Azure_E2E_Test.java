...
public class DSO_Azure_E2E_Test extends BaseUITest {

    private runManagerDataHandler dataBuilder;
    private DSOFlows dsoFlows;
    private static final String CLOUD = System.getProperty("cloud", "Azure").toLowerCase(Locale.ROOT);
    ...
    @DataProvider(name = "DSO_Azure_RunManager")
    public Object[][] getData() {
        return DSO_DataProvider.getExcelData(FILE_PATH, SHEET_NAME);
    }

    @Test(dataProvider = "DSO_Azure_RunManager",groups = "DSO_Azure")
    public void devSecOps_Azure_Combinations_Test(Map<String, String> testData) {
        DSOConstants data = dataBuilder.build(testData);
        dsoFlows.runAzureCombinationFlow(data);
    }
